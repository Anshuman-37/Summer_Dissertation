...
def get_mri_data(path_mri):
    ...
    '''
    path = path_mri;regex = re.compile(r'__MPR.*') ; fileRegex1 = re.compile(r'(WIP)*T13D'); mri_data_path = []; 
...
def get_asl_data(path_asl):
    ...
    '''
    asl_path = path_asl; asl_regex = re.compile(r'oxasl_dist.*'); asl_data_path = [];
    # Iterating over directories
...
def create_data_dict(mri_data_path,asl_data_path): 
    ...
    for i in asl_data_path:
        ## Value is our path i.e. stored and key is the patient number
        value = i; key = i.split('/')[5].split(' ')[0];asl_map[key] = value;
